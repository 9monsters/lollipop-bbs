...
<script type="text/javascript">
	var userId = "${sessionScope.user.id}";  //当前登录用户
	var createUserId = "${articleVo.createPerson.id}";  //发帖人
	...
	$(function(){
		...
		$("#submit").click(function(){
			if (CommonUtils.initValidator({form:'articleForm'}).form())
			{
				...
				var url = "${pageContext.request.contextPath}/IndexAjaxAction!updateArticle.action";
		...
		<form id="articleForm">
			<table class="tablebox border" cellSpacing="0" cellPadding="0" style="width:100%;">
				...
				<tr id="upload_input">
					...
					<td style="background: rgb(250, 253, 255); padding: 5px; border-bottom-color: rgb(228, 231, 236); border-bottom-width: 1px; border-bottom-style: solid;">
						<s:if test="articleVo.isThemeArticle == 1">
							<input id="title" class="FormClass" name="articleVo.title" value="${articleVo.title}" size="45">
				...
				<tr>
					<td style="background: rgb(250, 253, 255); padding: 10px 20px;" class="infoleft leftcolor" vAlign="top" align="left">
						<font color=red>*</font>帖子;容：
					</td>
					<td style="margin: 0px; padding: 0px;">
						<textarea name="content" style="width: 100%; height: 200px; visibility: hidden;">${articleVo.content}</textarea>
					</td>
				</tr>
				<tr>
					<td style="text-align: center; padding:5px;" class="tablebody2" colspan="2">
						<input id="submit" class="input0" value="  修  改  " type="button" />&nbsp;
						<input class="input0" onclick="window.close();" value="  取 消  " type="button" />
						<input type="hidden" name="articleVo.id" value="${articleVo.id}" />
					</td>
				</tr>
				</tbody>
			</table>
